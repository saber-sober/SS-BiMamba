...
def loadData():
    if config.data == 'PaviaU':
        data = sio.loadmat('/root/data/PaviaU/PaviaU.mat')['paviaU']
        labels = sio.loadmat('/root/data/PaviaU/PaviaU_gt.mat')['paviaU_gt']

    elif config.data == 'Indian':
        data = sio.loadmat('/root/data/Indian/Indian_pines_corrected.mat')['indian_pines_corrected']
        labels = sio.loadmat('/root/data/Indian/Indian_pines_gt.mat')['indian_pines_gt']

    elif config.data == 'Houston2013':
        data = sio.loadmat('/root/data/Houston2013/Houston.mat')['Houston']
        labels = sio.loadmat('/root/data/Houston2013/Houston_GT.mat')['Houston_GT']
    return data, labels
...
if __name__ == '__main__':
    ...
    for num in range(config.test_epoch):
        ...
        #     15:(254, 254, 0),
        # }
        # y_pred_all, yyy = mytest(net, all_data_loader)
        # img = np.zeros((y.shape[0]*y.shape[1], 3), dtype=np.int16)-1
        # img[index, 0] = y_pred_all
        # for i in range(img.shape[0]):
        #     key = img[i, 0]
        #     img[i, :] = colors[key]
        # img = img.reshape(y.shape[0], y.shape[1], 3)
